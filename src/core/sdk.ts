...
export class WebAASDK {
  ...
  private _runtimeMode: 'agent' | 'skill_provider' = 'agent';
  private _providerSocket: any = null;
  private _providerOptions: InitOptions | null = null;
  ...
  async init(options: InitOptions): Promise<void> {
    ...
    if (this._runtimeMode === 'skill_provider') {
      if (!options.providerId) throw new Error('skill_provider mode requires providerId');
      if (skills.length === 0) throw new Error('skill_provider mode requires at least one skill');
  ...
  private async _startSkillProvider(options: InitOptions): Promise<void> {
    this._providerOptions = options;
    const WebSocketImpl: any = typeof globalThis.WebSocket !== 'undefined'
      ? globalThis.WebSocket
      : (await (new Function('specifier', 'return import(specifier)') as (specifier: string) => Promise<any>)('ws')).default;
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const onMessage = async (event: any) => {
        ...
        if (message.type !== 'skill.execute') return;
      ...
      if (typeof socket.addEventListener === 'function') {
        ...
        socket.on('message', (data: unknown) => onMessage({ data }));
  ...
  disconnect(): void {
    ...
    if (this._providerSocket) {
      try { this._providerSocket.close(); } catch { /* ignore */ }
